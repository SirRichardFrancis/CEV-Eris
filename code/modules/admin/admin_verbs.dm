...
// GLOBAL_PROTECT(admin_verbs_debug_extra) // Especially niche verbs, enabled from debug panel
// GLOBAL_PROTECT(admin_verbs_permissions)
// GLOBAL_PROTECT(admin_verbs_mentor)
// GLOBAL_PROTECT(admin_verbs_mod)
// GLOBAL_PROTECT(admin_verbs_admin)

/client/proc/add_admin_verbs()
	if(holder)
		add_verb(src, GLOB.admin_verbs_default)
		if(BITTEST(holder.rights, R_ADMIN)) // Admin includes all moderator verbs
			add_verb(src, GLOB.admin_verbs_admin)
		else if(BITTEST(holder.rights, R_MOD)) // Mod includes all mentor verbs
			add_verb(src, GLOB.admin_verbs_mod)
		else if(BITTEST(holder.rights, R_MENTOR))
			add_verb(src, GLOB.admin_verbs_mentor)

		if(BITTEST(holder.rights, R_FUN))
			add_verb(src, GLOB.admin_verbs_fun)
		if(BITTEST(holder.rights, R_SERVER))
			add_verb(src, GLOB.admin_verbs_server)
		if(BITTEST(holder.rights, R_DEBUG))
			add_verb(src, GLOB.admin_verbs_debug)
		if(BITTEST(holder.rights, R_PERMISSIONS))
